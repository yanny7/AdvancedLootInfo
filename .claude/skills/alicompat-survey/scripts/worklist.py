...
def main():
    ...
    for index, row in enumerate(rows, 1):
        ...
        add("- [ ] `supported_mods.json` entry, then `check_versions.py --scaffold <slug>`")
    ...
    add("`supported_mods.json` entry, `check_versions.py --scaffold <slug>` for the `compat_mods`")
    add("entry, the `<slug>_<loader>_dep` lines and the scaffolded `src/compat/<slug>/` source set, the real")
