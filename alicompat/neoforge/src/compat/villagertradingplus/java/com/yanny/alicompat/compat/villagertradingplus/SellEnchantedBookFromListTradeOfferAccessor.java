...
public class SellEnchantedBookFromListTradeOfferAccessor extends BaseAccessor<VillagerTrades.ItemListing> implements IItemListing {
    ...
    public IDataNode getNode(IServerUtils utils, TooltipNode conditions) {
        TooltipNode tooltip = TooltipBuilder.branch((b) -> entries.forEach((e) -> {
            ...
            b.add(TooltipBuilder.array((c) -> {
                c.add(utils.getValueTooltip(utils, getEnchantment(utils, entry)).build(Lang.Value.ENCHANTMENT));
    ...
    @NotNull
    private RangeValue getCostRange(IServerUtils utils) {
        int min = MAX_COST;
        int max = MIN_COST;
        ...
        for (Object e : entries) {
            ...
            min = Math.min(min, getCost(getEnchantment(utils, entry), entry.getMinLevel()));
            max = Math.max(max, getCost(getEnchantment(utils, entry), entry.getMaxLevel()));
        }

        return new RangeValue(Math.min(min, max), max);
    }

    private int getCost(@Nullable Holder<Enchantment> enchantment, int level) {
        int cost = baseCost + level * costPerLevel;

        if (enchantment != null && enchantment.is(EnchantmentTags.TREASURE)) {
    ...
    @Nullable
    private static Holder<Enchantment> getEnchantment(IServerUtils utils, EnchantmentEntryAccessor entry) {
        return utils.lookupProvider().lookupOrThrow(Registries.ENCHANTMENT).get(entry.getKey()).map((holder) -> (Holder<Enchantment>) holder).orElse(null);
    }
}
