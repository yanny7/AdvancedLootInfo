...
public class JeiTradeLoot extends JeiBaseLoot<TradeLootType, ResourceLocation> {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, RecipeHolder<TradeLootType> recipe, IFocusGroup iFocusGroup) {
        ...
        if (header.spawnEgg() != null) {
            ...
            builder.addSlot(RecipeIngredientRole.CATALYST).setSlotName(SPAWN_EGG).setPosition(rect.x(), rect.y()).setStandardSlotBackground().addItemLike(header.spawnEgg());
        }
    ...
    private static void addInputSlots(IRecipeLayoutBuilder builder, String name, Set<? extends ItemLike> items, List<Rect> rects) {
        ...
        for (ItemLike item : items) {
            ...
            builder.addInputSlot().setSlotName(name + i).setPosition(rect.x(), rect.y()).setStandardSlotBackground().addItemLike(item);
            i++;
        }
    }
