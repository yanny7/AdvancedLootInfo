...
public class ReiTradeCategory extends ReiBaseCategory<ReiTradeDisplay, ResourceLocation> {
    ...
    public List<Widget> setupDisplay(ReiTradeDisplay display, Rectangle bounds) {
        ...
        Label title = Widgets.createLabel(new Point(titleRect.x(), titleRect.y()), header.title()).leftAligned().noShadow().color(0);
        ...
        if (!display.getType().accepts().isEmpty()) {
            ...
            innerWidgets.add(Widgets.createLabel(new Point(labelRect.x(), labelRect.y()), header.acceptsLabel()).leftAligned().noShadow().color(0));
