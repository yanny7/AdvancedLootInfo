...
public class JeiEntityLoot extends JeiBaseLoot<EntityLootType, Entity> {
    ...
    public void draw(EntityLootType recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        ...
        guiGraphics.drawString(font, recipe.entity().getDisplayName(), (9 * 18 - font.width(recipe.entity().getDisplayName())) / 2, 0, 0, false);
        GenericUtils.renderEntity(recipe.entity(), new Rect((9 * 18 - WIDGET_SIZE) / 2, 10, WIDGET_SIZE, WIDGET_SIZE), 9 * 18, guiGraphics, (int) mouseX, (int) mouseY);
    }
