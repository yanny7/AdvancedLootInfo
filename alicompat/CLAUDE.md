...
- `farmersdelight` (Fabric + Forge) — `CopySkilletFunction`/`CopyMealFunction` tooltips (keys in `FarmersDelightLang`) and the four Farmer's Delight GLMs (`AddItemModifier`, `AddLootTableModifier`, `PastrySlicingModifier`, `ReplaceItemModifier`). The two source sets are otherwise identical, but only the Fabric one registers an item listing: Fabric's Farmer's Delight wraps each villager trade in a private `VillagerEvents$FDItemListing` record gating it on a config flag, so `FDItemListingAccessor` unwraps the delegate listing and hands it back to `IServerUtils.getItemListing`. Forge's Farmer's Delight builds the same trades out of Forge's `BasicItemListing`, which `ali/forge` already covers.
- `portinglib` (Fabric only) — two compats over two Porting Lib modules. `PortingLibCompat` (`porting_lib_tool_actions`) is the `CanToolPerformAction` condition tooltip; Forge needs no shim, `ali/forge` covers Forge's native `CanToolPerformAction` through `MixinCanToolPerformAction`. `PortingLibLootCompat` (`porting_lib_loot`) is ALI's **entire** Global Loot Modifier path on Fabric — Fabric has no GLM machinery of its own, so ALI reads Porting Lib's: the compat reads `LootModifierManager`'s modifier map, hands every `IGlobalLootModifierPlugin` among ALI's plugins (this jar's own `Plugin` included) a registry to declare its adapters into, and falls back to `GlobalLootModifierUtils.getMissingGlobalLootModifier` for modifier classes nobody wrote an adapter for. It also registers the destination resolver for Porting Lib's `LootTableIdCondition`, the Fabric counterpart of the one `ali/forge` registers for Forge's. Without ALICompat installed, a Fabric client therefore shows loot unmodified by any GLM — including the Farmer's Delight ones above, whose adapters live here too.
