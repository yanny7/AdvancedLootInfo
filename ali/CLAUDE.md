...
- There is **no per-listing item collector**: a trade's two sides come from `ITradeNode`, so nothing in the API answers "what items does this `ItemListing` produce". `registerItemListing` (the node factory) is the only trade-side registration.
