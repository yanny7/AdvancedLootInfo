...
def main():
    ...
    for index, row in enumerate(rows, 1):
        ...
        add("- [ ] `supported_mods.json` entry + `compat_mods` entry, then `check_versions.py --update`")
    ...
    add("")
    add("Suggested property names (mod id with separators stripped); a slug already in")
    add("`supported_mods.json` keeps its existing name. Pick the block for the branch you are on.")
    add("The slug and project id out of each coordinate are what the registry entry needs; the file")
    add("id is `check_versions.py`'s business.")
    add("")
    ...
    add("")
    add("Per-mod steps follow `alicompat/CLAUDE.md`'s \"Adding a target mod\": the")
    add("`supported_mods.json` entry and `compat_mods` entry, `check_versions.py --update` for the")
    add("`<slug>_<loader>_dep` lines and the scaffolded `src/compat/<slug>/` source set, the real")
    add("`IModCompat` implementation and its `services` fragment, then the tooltip keys and datagen")
    add("if the shim introduces any.")
    add("")
    add("## Queue")
    add("")
