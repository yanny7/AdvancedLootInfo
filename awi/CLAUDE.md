...
`MAX_METHODS` is sized against all of that: a runaway guard at ~2× vanilla's widest feature (`fossil`, 422 methods), not a tuning knob — lowering it silently drops blocks rather than failing.
...
The scanner's output is guarded by `FeatureBytecodeScanTest` (golden file `feature_bytecode_scan.json`), which also records per-feature `MAX_METHODS` consumption. Widening the walk must only *add* blocks and must leave `methodLimitReached` false everywhere; block tags resolve to nothing under the test bootstrap, so the file is a differential baseline, not a coverage claim.
