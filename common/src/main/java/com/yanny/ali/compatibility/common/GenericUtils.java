...
//            decompressedBuf.writeBytes(gzip, gzip.available());
        ...
        try {
            ...
            tradeData.putAll(readerBuf.readCollection(ArrayList::new, (b) -> {
                ...
                List<Item> inputs = b.readCollection(ArrayList::new, FriendlyByteBuf::readResourceLocation).stream().map(BuiltInRegistries.ITEM::getValue).toList();
                List<Item> outputs = b.readCollection(ArrayList::new, FriendlyByteBuf::readResourceLocation).stream().map(BuiltInRegistries.ITEM::getValue).toList();
