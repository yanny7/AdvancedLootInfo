...
- `AliConfig` — root config (`configVersion`, `blockCategories/entityCategories/gameplayCategories/tradeCategories` lists, `disabledEntities`, `logMoreStatistics`, `showInGameNames`); ships hardcoded defaults (`plant_loot`, `block_loot`, `entity_loot`, `chest_loot`, `fishing_loot`, `archaeology_loot`, `hero_loot`, `cat_morning_gift`, `piglin_bartering`, `sniffer_digging`, `gameplay_loot`, `trade_loot`).
