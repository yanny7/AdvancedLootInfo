...
1. `supported_mods.json` in the repo root: add an entry with the slug (`key`), the display `name`, the runtime `mod_ids` the shim gates on, and the mod's CurseForge projects as `<slug>-<project id>` — one, or one per project when the mod is published separately per loader. Loaders are not listed: which project has a file for which loader is resolved against CurseForge per Minecraft version. It is the Minecraft-version-agnostic registry of everything ALICompat supports, described by `supported_mods.schema.json`, and both `check_versions.py` and `modpack.py` read it as their source of truth.
