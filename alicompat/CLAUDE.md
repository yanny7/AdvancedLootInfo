...
- `farmersdelight` (Fabric + Forge) — `CopySkilletFunction`/`CopyMealFunction` tooltips (keys in `FarmersDelightLang`) and the four Farmer's Delight GLMs (`AddItemModifier`, `AddLootTableModifier`, `PastrySlicingModifier`, `ReplaceItemModifier`). The two source sets are identical except for the target mod's own coordinates.
- `portinglib` (Fabric only) — the `CanToolPerformAction` condition tooltip. Forge needs no shim: `ali/forge` covers Forge's native `CanToolPerformAction` through `MixinCanToolPerformAction`.
