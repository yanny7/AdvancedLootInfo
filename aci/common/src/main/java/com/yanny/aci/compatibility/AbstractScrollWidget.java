...
public abstract class AbstractScrollWidget {
    ...
    public void render(GuiGraphics guiGraphics, double mouseX, double mouseY) {
        guiGraphics.blitNineSliced(getTexture(), scrollRect.x(), scrollRect.y(), scrollRect.width(), scrollRect.height(), 2, 16, 16, 2, 2);

        Rect markerArea = calculateScrollbarMarkerArea();
        guiGraphics.blitNineSliced(getTexture(), markerArea.x(), markerArea.y(), markerArea.width(), markerArea.height(), 2, 2, 2, 1, 12, 17, 18, 0);
