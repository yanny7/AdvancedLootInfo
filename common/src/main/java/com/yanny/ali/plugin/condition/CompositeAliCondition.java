...
public abstract class CompositeAliCondition implements ILootCondition {
    public final List<ILootCondition> terms;

    public CompositeAliCondition(IContext context, CompositeLootItemCondition condition) {
        terms = context.utils().convertConditions(context, ((MixinCompositeLootItemCondition) condition).getTerms());
    }

    public CompositeAliCondition(IContext context, FriendlyByteBuf buf) {
