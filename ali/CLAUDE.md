...
- `AliServerRegistry` — the workhorse: entry/condition/function tooltip builders, value tooltips, chance/count/itemstack modifiers, item listings, and the loot-table/trade parsing entry points `parseTable`/`parseTrade`, plus the tooltip cache.
...
- `registerClient` — registers a widget + data-node factory pair for every node ID: loot side (`LootTableNode/EntityLootTableNode/LootPoolNode/ItemNode/EmptyNode/ReferenceNode/DynamicNode/AlternativesNode/SequenceNode/GroupNode/ModifiedNode/GlobalLootModifierNode/MissingNode`) and trade side (`TradeNode/TradeLevelNode/SubTradesNode/ItemsToItemsNode`).
- `registerServer` — the bulk of the file: number-provider→`RangeValue` converters (`ConstantValue, UniformGenerator, BinomialDistributionGenerator, ScoreboardValue`), entry→node builders, 8 entry tooltips (`EntryTooltipUtils`), ~18 condition tooltips and ~24 function tooltips (`ConditionTooltipUtils`/`FunctionTooltipUtils`), an ingredient tooltip, ~35 value tooltips (vanilla-registry values via `RegistriesTooltipUtils`, e.g. `Block/Item/EntityType/Enchantment/Attribute`; structural predicate/misc values via `ValueTooltipUtils`, e.g. `EntityPredicate/ItemPredicate/NumberProvider/MinMaxBounds`), chance/count/item-stack modifier appliers, the two vanilla global-loot-modifier destination resolvers, and villager-trade `IItemListing` registrations covering all 10 `VillagerTrades.*` inner classes (most via generic `TradeUtils`; `EmeraldsForVillagerTypeItem` gets a dedicated node).
...
Client-side rendering. `ClientUtils`/`WidgetUtils` helpers; one `ListWidget`/`IWidgetFactory` per node ID mirroring `plugin/common/nodes` (`LootTableWidget, LootPoolWidget, ItemWidget, AlternativesWidget, SequentialWidget, GroupWidget, EmptyWidget, DynamicWidget, ReferenceWidget, ModifiedWidget, GlobalLootModifierWidget, MissingWidget, TextureWidget`), plus `widget/trades` (`TradeWidget, TradeLevelWidget, SubTradesWidget, ItemListingWidget`). The pairing is one widget per node ID rather than per class - `EntityLootTableNode` renders through `LootTableWidget`, the same widget its superclass uses.

### `plugin/glm` — Global Loot Modifier compatibility
Forge/NeoForge GLM datapack modifiers layer: `IGlobalLootModifierPlugin` (extra `IPlugin` extension GLM-integrating mods implement, `registerGlobalLootModifier`), `IGlobalLootModifierWrapper` (loader-agnostic view over one GLM), `Destination`/`IDestinationResolver` (what a modifier's conditions point at), `GlobalLootModifierUtils` (turns a GLM's conditions plus its operations into an `ILootModifier` bound to blocks, entities or one loot table, with `getMissingGlobalLootModifier` as the fallback for modifier classes no plugin adapts).

Destination detection is registry-driven: a resolver registered per `LootItemCondition` class through `IServerRegistry.registerDestination` answers a `Destination.Blocks`/`Entities`/`Table` (or `null` for a condition that names no destination), and `IServerUtils.getDestination` dispatches on the condition's exact class. A condition whose own class has no resolver is descended into when it is a `CompositeLootItemCondition` (`AllOfCondition`/`AnyOfCondition`), so nested destinations are found. The vanilla resolvers live in `plugin/Plugin` (`LootItemBlockStatePropertyCondition` → blocks; `LootItemEntityPropertyCondition` → entities, only for `EntityTarget.THIS` with a non-`ANY` entity type); the loot-table-id condition is loader-specific and registered by `ali/forge`'s `ForgePlugin` on Forge and by ALICompat's `portinglib` compat on Fabric.

Each `Destination` carries `fullyExplained` — true when the condition says nothing beyond naming the destination. Only a fully-explained condition is dropped from the operations rendered in the tooltip, so a block-state condition that also constrains properties, or an entity predicate carrying more than the type, stays visible on the page it selected.

Kinds are tried in order — entities, then blocks, then loot table — and the first kind any condition resolves to fixes the modifier's `ILootModifier.IType`. A modifier nothing resolves for becomes `IType.UNBOUNDED`, which matches every page, and is produced only when `showUnboundedGlobalLootModifiers` is enabled; otherwise it is dropped.

`AbstractServer.readLootTables` groups the collected modifiers by `IType` and attaches them per page. Candidate items for a page come from parsing its table (plus any `fake_loot` pools for it) with no modifiers applied; a bound modifier survives when one of those items matches one of its operations **or** the page yields no candidates at all, while an `UNBOUNDED` one needs a real item match. Modifiers that end up on no page are counted into the log.
...
- `ConfigUtils` — reads/writes `config/ali/ali_common.json` (`Utils.COMMON_CONFIG_NAME`), handles `CURRENT_VERSION` migration by renaming the outdated file to `.bak` and regenerating defaults. The read/rotate/re-create logic itself is `aci.configuration.CoreConfigUtils` (shared with AWI); this class only supplies the Gson with its `LootCategoryAdapter`. `AliConfig` implements `aci.configuration.ICoreConfig`, and its `normalize()` is what turns an explicit `null` in the file back into the default value.
...
- The server still walks the tree (`AbstractServer.collectItems`) for two things only: telling an empty loot table from a real one in `removeEmptyLootTable`, and collecting a page's candidate items to match global loot modifiers against (see `plugin/glm`). Neither list is sent.
- There is **no item collector registration at all**, on either side: a table's items come from walking its parsed nodes and a trade's two sides from `ITradeNode`, so nothing in the API answers "what items does this entry/function/`ItemListing` produce". `registerItemListing` (the node factory) is the only trade-side registration.
