...
public class BlockNode implements IDataNode, IBlockNode {
    ...
    public BlockNode(IClientUtils utils, FriendlyByteBuf buf) {
        block = buf.readBoolean()
                ? Either.left(BuiltInRegistries.BLOCK.getValue(buf.readResourceLocation()))
                : Either.right(TagKey.create(Registries.BLOCK, buf.readResourceLocation()));
    ...
    public void encode(IServerUtils utils, RegistryFriendlyByteBuf buf) {
        block.ifLeft((value) -> {
            ...
            buf.writeResourceLocation(BuiltInRegistries.BLOCK.getKey(value));
        }).ifRight((tag) -> {
