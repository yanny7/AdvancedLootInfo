...
import com.yanny.ali.lootjs.widget.*;
        ...
        for (ILootAction action : actions) {
            if (action instanceof AbstractLootModification lootModification) {
                try {
                    ...
                    LOGGER.error("Failed to process loot modification {}: {}", ((MixinAbstractLootModification) lootModification).getName(), e.getMessage(), e);
                }
            } else {
