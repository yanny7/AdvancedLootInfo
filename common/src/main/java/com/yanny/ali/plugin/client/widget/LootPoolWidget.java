...
public class LootPoolWidget implements IWidget {
    ...
    public LootPoolWidget(IWidgetUtils utils, LootPool entry, int x, int y, int maxWidth, List<LootItemFunction> functions, List<LootItemCondition> conditions) {
        ...
        Pair<List<IEntryWidget>, Rect> info = utils.createWidgets(utils, entry.entries, x, y, maxWidth, List.copyOf(allFunctions), List.copyOf(allConditions));
    ...
    public static Rect getBounds(IClientUtils registry, LootPool entry, int x, int y, int maxWidth) {
        return registry.getBounds(registry, entry.entries, x, y, maxWidth);
    }
}
