...
public abstract class JeiBaseLoot<T extends IType, V> implements IRecipeCategory<T> {
    ...
    private void setupTooltip(ITooltipBuilder tooltipBuilder, SingletonEntry entry, RangeValue chance, @Nullable Pair<Enchantment, Map<Integer, RangeValue>> bonusChance, RangeValue count,
                              @Nullable Pair<Enchantment, Map<Integer, RangeValue>> bonusCount, List<ILootFunction> functions, List<ILootCondition> conditions) {
        tooltipBuilder.addAll(TooltipUtils.getQuality(entry));
