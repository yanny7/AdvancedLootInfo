...
public class EmiScrollWidget extends Widget implements IMouseEvents {
    ...
    public void render(GuiGraphics draw, int mouseX, int mouseY, float delta) {
        draw.blitNineSliced(WidgetUtils.TEXTURE_LOC, scrollRect.x(), scrollRect.y(), scrollRect.width(), scrollRect.height(), 2, 16, 16, 2, 2);

        Rect markerArea = calculateScrollbarMarkerArea();
        draw.blitNineSliced(WidgetUtils.TEXTURE_LOC, markerArea.x(), markerArea.y(), markerArea.width(), markerArea.height(), 2, 2, 2, 1, 12, 17, 18, 0);
