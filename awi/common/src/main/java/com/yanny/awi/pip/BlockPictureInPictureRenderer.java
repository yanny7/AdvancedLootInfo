...
public final class BlockPictureInPictureRenderer extends PictureInPictureRenderer<BlockRenderState> {
    ...
    protected void renderToTexture(BlockRenderState renderState, PoseStack poseStack) {
        ...
        // apply already ends with a translate(-0.5) that centres the 0..1 model on the origin, which is the texture's
        // centre - do not add another one here.
        ...
        Minecraft.getInstance().getBlockRenderer().renderSingleBlock(
                state, poseStack, bufferSource, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY
        );
    }
    ...
    @NotNull @Override
    protected String getTextureLabel() { return "block block-in-ui"; }

    @NotNull @Override
    public Class<BlockRenderState> getRenderStateClass() { return BlockRenderState.class; }
}
