...
public class EmiBlockLoot extends EmiBaseLoot {
    ...
    public EmiBlockLoot(EmiRecipeCategory category, Block block, IDataNode lootTable, List<ItemStack> outputs) {
        ...
        super(category, getRecipeId(block), lootTable, 0, (block instanceof VegetationBlock || block.asItem() == Items.AIR) ? 30 : 22, Collections.emptyList(), outputs);
        this.block = block;
    ...
    private static ResourceLocation getRecipeId(Block block) {
        ...
        return ResourceLocation.fromNamespaceAndPath(location.getNamespace(), "blocks/" + location.getPath());
    }
}
