...
//    private static final List<Potion> POTIONS = BuiltInRegistries.POTION.stream().filter((potion) -> !potion.getEffects().isEmpty() && PotionBrewing.isBrewablePotion(potion)).toList();
    ...
    public IDataNode getNode(IServerUtils utils, ITooltipNode conditions) {
        return new ItemsToItemsNode(
                ...
                Either.left(sell),
                new RangeValue(),
                EmptyTooltipNode.EMPTY, // List.of(GenericTooltipUtils.getCollectionTooltip(utils, "ali.property.branch.effects", "aci.util.null", POTIONS, RegistriesTooltipUtils::getPotionTooltip)),
                maxUses,
                experience,
                multiplier,
                conditions
