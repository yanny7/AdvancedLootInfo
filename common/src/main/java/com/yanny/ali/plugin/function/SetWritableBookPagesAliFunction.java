...
public class SetWritableBookPagesAliFunction extends LootConditionalAliFunction {
    public final List<Filterable<Component>> pages;
    public final String operation;

    public SetWritableBookPagesAliFunction(IContext context, LootItemFunction function) {
    ...
    public SetWritableBookPagesAliFunction(IContext context, FriendlyByteBuf buf) {
        super(context, buf);
...
/* FIXME
        ...
        if (!pages.isEmpty()) {
            pages.forEach((page) -> {
                components.add(pad(pad + 1, translatable("ali.property.function.set_writable_book_pages.pages")));
                components.add(pad(pad + 2, translatable("ali.property.function.set_writable_book_pages.pages.page", page.raw())));
                page.filtered().ifPresent((f) -> components.add(pad(pad + 3, translatable("ali.property.function.set_writable_book_pages.pages.page.filtered", f))));
            });
        }

        components.add(pad(pad + 1, translatable("ali.property.function.set_writable_book_pages.page_operation", operation)));
*/
        return components;
    }
}
