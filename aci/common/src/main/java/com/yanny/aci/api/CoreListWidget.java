...
public abstract class CoreListWidget<
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.blitRepeating(getTexture(), bounds.getX() + groupWidgetWidth / 2, top, 2, height, 0, 0, 2, 18);
        lastDirection = null;
        ...
        for (IWidget widget : widgets) {
            ...
            if ((direction == WidgetDirection.VERTICAL || (lastDirection != null && direction != lastDirection)) && widget.getRect().getOffsetY() > 0) {
                guiGraphics.blitRepeating(getTexture(), (int) (bounds.getX() + Math.floor((double) groupWidgetWidth / 2) + 1), widget.getRect().getY() + 8, (int) (Math.ceil((double) groupWidgetWidth / 2) - 1), 2, 2, 0, 18, 2);
            }
